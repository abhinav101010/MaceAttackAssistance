 ...
 *  net.minecraft.SimpleParticleType
 *  net.minecraft.Vec3d
...
public class MaceParticle {
    ...
    private static Vec3d getWeaponPosition(MinecraftClient client, ClientPlayerEntity player) {
        ...
        double pitch;
        Vec3d playerPos;
        if (client.options.getPerspective().equals((Object)Perspective.FIRST_PERSON)) {
        ...
        return playerPos.add(MaceParticle.getOffset(pitch, yaw, getDistance));
    }

    private static Vec3d getOffset(double pitch, double yaw, double getDistance) {
        ...
        return new Vec3d(offsetX * getDistance, offsetY * getDistance, offsetZ * getDistance);
    }
}
