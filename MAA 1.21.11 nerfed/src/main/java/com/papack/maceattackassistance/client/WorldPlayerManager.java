...
public class WorldPlayerManager {
    ...
    public static void setWorldPlayers(MinecraftClient client) {
        ClientPlayNetworkHandler networkHandler = client.getNetworkHandler();
        if (networkHandler != null) {
            Collection<PlayerListEntry> entryCollection = networkHandler.getListedPlayerListEntries();
            for (PlayerListEntry entry : entryCollection) {
                if (entry == null) continue;
                UUID uuid = entry.getProfile().id();
                WorldPlayerManager.addPlayer(entry.getProfile().name(), uuid, entry.getSkinTextures(), entry.getScoreboardTeam(), FriendManager.isFriend(uuid));
            }
        }
    }

    public static void addPlayer(String name, UUID uuid, SkinTextures skin, Team team, boolean friend) {
