...
public class BeamRenderHandler {
    ...
    private static void renderBeam(MatrixStack matrices, VertexConsumerProvider consumers, int targetColor) {
        float r = (float)ColorHelper.Argb.getRed((int)targetColor) / 255.0f;
        float g = (float)ColorHelper.Argb.getGreen((int)targetColor) / 255.0f;
        float b = (float)ColorHelper.Argb.getBlue((int)targetColor) / 255.0f;
        float alpha = 1.0f;
        float height = 2.5f;
        float width = 0.1f;
