            ...
            try {
                if (target instanceof LivingEntity) {
                    ...
                    if (MaceAttackAssistanceClient.shouldShieldBreak && !mainHandItemIsAxe && (axeSlot = StunSlam.getAxeSlotId(clientPlayer)) > -1) {
                        MaceAttackAssistanceClient.ex_preStun_slot = MaceAttackAssistanceClient.ex_previous_slot == -1 ? playerInventory.selectedSlot : MaceAttackAssistanceClient.ex_previous_slot;
                        playerInventory.selectedSlot = axeSlot;
