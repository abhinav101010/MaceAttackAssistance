...
public class BeamRenderHandler {
    private static final Identifier GLOW_TEXTURE = Identifier.of("maceattackassistance", "textures/glow_particle.png");
    ...
    public static List<Entity> getWorldEntityList(MinecraftClient client, ClientPlayerEntity clientPlayer, World world) {
        ...
        if (client.world != null) {
            worldEntities = client.world.getOtherEntities((Entity)clientPlayer, searchArea, entity -> entity.isLiving() && entity.isAlive() && MaceAttackAssistanceClient.isAllowedTarget(entity) && !BeamRenderHandler.isOutsideViewCone(playerEyePos, new Vec3d(entity.getX(), entity.getY(), entity.getZ()), playerYaw, playerPitch, yawFovDegrees, pitchFovDegrees) && MaceAttackAssistanceClient.isSimpleVisibleFromPlayer(client, clientPlayer, entity));
        }
    ...
    private static void renderBeam(MatrixStack matrices, VertexConsumerProvider consumers, int targetColor) {
        float r = (float)ColorHelper.getRed(targetColor) / 255.0f;
        float g = (float)ColorHelper.getGreen(targetColor) / 255.0f;
        float b = (float)ColorHelper.getBlue(targetColor) / 255.0f;
        float alpha = 1.0f;
        float height = 2.5f;
        float width = 0.1f;
        MatrixStack.Entry entry = matrices.peek();
        Matrix4f model = entry.getPositionMatrix();
        VertexConsumer vc = consumers.getBuffer(RenderLayers.lightning());
        for (int i = 0; i < 4; ++i) {
    ...
    private static void renderSpiralParticlesTrail(MatrixStack matrices, VertexConsumerProvider consumers, int targetColor, Entity targetEntity) {
        ...
        float g = (float)ColorHelper.getGreen(targetColor) / 255.0f;
        float b = (float)ColorHelper.getBlue(targetColor) / 255.0f;
        VertexConsumer vc = consumers.getBuffer(RenderLayers.entityTranslucentEmissive(GLOW_TEXTURE));
        ...
        for (int s = 0; s < spiralCount; ++s) {
            ...
            Deque<Vector3f> trail = (Deque<Vector3f>)trails.get(s);
