...
public class ElytraBoost {
    ...
    public static void elytraBoost(MinecraftClient client, ClientPlayerEntity clientPlayer) {
        ...
        if (Config.JUMP_ASSIST && cooldownCounter == 0 && clientPlayer.isGliding() && client.options.jumpKey.isPressed() && (!client.options.attackKey.isPressed() || client.targetedEntity == null)) {
    ...
    public static boolean isNotUsableItems(MinecraftClient client, ItemStack itemStack) {
        ...
        if ((translationKey.contains("helmet") || translationKey.contains("chestplate") || 
             translationKey.contains("leggings") || translationKey.contains("boots")) && 
            !itemStack.isOf(Items.WOLF_ARMOR)) {
            return false;
        }
